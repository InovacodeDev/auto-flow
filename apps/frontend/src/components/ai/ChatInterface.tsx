...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({ userId, organizationId, industry }) => {
    ...
    const { messages, isTyping, sendMessage, clearMessages, isLoading, error } = useChatSession(sessionId);
        ...
        <div className="flex flex-col h-full bg-white">
            ...
            <div className="flex items-center justify-between p-4 border-b border-gray-200 bg-gray-50">
                <div className="flex items-center space-x-3">
                    ...
                    <div>
                        <h2 className="text-lg font-semibold text-gray-900">Alex - Assistente IA</h2>
                ...
                <div className="flex items-center space-x-2">
                    ...
                    >
                        <MaterialIcon icon="delete" className="text-current" size={20} />
                    </button>
                    <button
                    ...
                    >
                        <MaterialIcon
                            icon="refresh"
                            className={`text-current ${startConversationMutation.isPending ? "animate-spin" : ""}`}
                            size={20}
                        />
                    </button>
                </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {messages.length === 0 && !isTyping && (
                    <div className="text-center py-8">
                        <MaterialIcon icon="auto_awesome" className="text-gray-400 mx-auto mb-4" size={48} />
                        <h3 className="text-lg font-medium text-gray-900 mb-2">Olá! Sou o Alex</h3>
                        <p className="text-gray-600">Seu assistente de automação. Como posso ajudá-lo hoje?</p>
                    </div>
                )}
                ...
                {error && (
                    <div className="flex items-center space-x-2 p-3 bg-red-50 border border-red-200 rounded-lg">
                        <MaterialIcon icon="warning" className="text-red-500 flex-shrink-0" size={20} />
                        <span className="text-sm text-red-700">Erro ao processar mensagem. Tente novamente.</span>
                    </div>
                )}
            ...
            {/* Suggestions Panel */}
            {showSuggestions && messages.length === 0 && <SuggestionsPanel onSuggestionClick={handleSuggestionClick} />}
            ...
            <div className="p-4 border-t border-gray-200 bg-gray-50">
                <form onSubmit={handleSendMessage} className="flex items-center space-x-3">
                    <div className="flex-1 relative">
                        ...
                        >
                            <MaterialIcon icon="send" className="text-current" size={20} />
                        </button>
                    </div>
                </form>
                ...
                {!isConnected && (
                    <div className="mt-2 flex items-center space-x-2 text-sm text-amber-600">
                        <MaterialIcon icon="warning" className="text-current" size={16} />
